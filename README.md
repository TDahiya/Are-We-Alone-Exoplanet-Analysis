# Are We Alone? The Search for Earth 2.0 🌍
> **Authors:** Tanishq Dahiya
> **Module:** CSC1143/CSC1175 Data Visualisation

## 📖 Overview
[cite_start]This project tackles one of the most profound questions in astronomy: *"Are we alone?"*[cite: 1, 63]. [cite_start]Using the **NASA Exoplanet Archive dataset** [cite: 65][cite_start], we analyzed the physical characteristics of thousands of exoplanets to determine the rarity of "Earth-like" worlds compared to uninhabitable gas giants[cite: 64].

[cite_start]The core of this project is a data visualization that maps planetary radius against orbital distance, highlighting a distinct "Goldilocks Zone" of potential habitability[cite: 6].

## 📊 Key Features & Analysis
- **Data Cleaning:** Processed raw NASA data using **Pandas**, handling missing values and filtering for relevant astrophysical parameters.
- **Visualization:** Created a complex scatter plot using **Matplotlib** with the following features:
  - [cite_start]**Log-Scaling:** Applied to handle the vast range of planetary distances[cite: 5].
  - **Inset Zoom:** A specialized zoom-in window to label specific candidate planets like *Kepler-452 b* that are statistically rare.
  - **Contextual Annotations:** Used green connecting lines to link the "Earth-like" cluster to the zoom window.

## 🛠️ Technologies Used
- **Language:** Python
- **Libraries:**
  - `pandas` (Data manipulation)
  - `matplotlib` (Visualization & Patches)
  - `numpy` (Numerical operations)

## 🚀 How to Run
1. Clone the repository:
   ```bash
   git clone [https://github.com/your-username/Are-We-Alone-Exoplanet-Analysis.git](https://github.com/your-username/Are-We-Alone-Exoplanet-Analysis.git)
